...
import ParametricGeometryHelper from "./helpers/ParametricGeometryHelper";
import CameraZoomHelper from "@/helpers/CameraZoomHelper";

@RenderLoop()
class ParametricGeometryProject extends BaseProject {
  declare private root: THREE.Group;
  declare private meshMaterial: THREE.Material;
  declare private lineMaterial: THREE.Material;
  declare private geometryHelper: ParametricGeometryHelper;

  setupModel() {
    if (!this.controlUI) throw new Error("controlUI가 없습니다.");

    this.geometryHelper = new ParametricGeometryHelper(this.controlUI);
  ...
  createModel() {
    ...
    const groupName = "ParametricModel";
...
export default ParametricGeometryProject;
